...
game_over_exit = myfont.render("Click anywhere to exit", True, black)
game_won_text = myfont.render("U WON!", True, black)
...
while True:
    ...
    if countdown <= 0:
        ...
        while gameover:
            ...
            screen.blit(game_over_exit, (280, 150))
        ...
        if event.type == pygame.KEYDOWN:  # this will stop printing mouse movement coordinate in console
            ...
            for i, rect in enumerate(difference_rectangle):
                if mouse_pos[0] > rect[0] and mouse_pos[0] < rect[0] + rect[2] \
                    ...
                    print("Got it")
                    found_difference.add(i)            # dont use append otherwise it will show won on pressing sameplace 3 times
    ...
    # for countdown bar
    pygame.draw.rect(screen, white, (1200/2 - 20, 0, 40, 450))           # white background of bar
    ...
    print(len(found_difference))
    if len(found_difference) == 3:
        gamewon = True
        while gamewon:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
            ...
            screen.fill(white)
            screen.blit(game_won_text, (250, 50))
            screen.blit(game_over_exit, (280, 150))
